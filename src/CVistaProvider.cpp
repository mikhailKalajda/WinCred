...
#include "CVistaProvider.h"
#include "CVistaCredential.h"
#include "Dll.h"
#include "guid.h"
#include "easylogging++.h"

CVistaProvider::CVistaProvider() :
    _cRef(1),
...
void CVistaProvider::_CleanupSetSerialization()
{
...
HRESULT CVistaProvider::SetUsageScenario(
...
STDMETHODIMP CVistaProvider::SetSerialization(
...
HRESULT CVistaProvider::UnAdvise()
{
    return E_NOTIMPL;
}
...
HRESULT CVistaProvider::GetFieldDescriptorCount(
    DWORD* pdwCount
)
{
...
HRESULT CVistaProvider::GetFieldDescriptorAt(
    DWORD dwIndex,
...
HRESULT CVistaProvider::GetCredentialCount(
    DWORD* pdwCount,
    DWORD* pdwDefault,
...
// the tiles.
HRESULT CVistaProvider::GetCredentialAt(
    DWORD dwIndex,
...
HRESULT CVistaProvider::_EnumerateOneCredential(
...
)
{
    ...
    CVistaCredential* ppc = new CVistaCredential();
...
HRESULT CVistaProvider::_EnumerateCredentials()
{
...
HRESULT CVistaProvider_CreateInstance(REFIID riid, void** ppv)
{
    LOG(INFO) << "CVistaProvider_CreateInstance";
    HRESULT hr;

    CVistaProvider* pProvider = new CVistaProvider();

    if (pProvider)
    {
        hr = pProvider->QueryInterface(riid, ppv);
        LOG(INFO) << "CVistaProvider_CreateInstance QueryInterface hr = " << hr;
        pProvider->Release();
    }
    else
    {
...
// more information.
HRESULT CVistaProvider::_EnumerateSetSerialization()
{
    KERB_INTERACTIVE_LOGON* pkil = &_pkiulSetSerialization->Logon;
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            CVistaCredential* pCred = new CVistaCredential();
